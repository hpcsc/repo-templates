...
# {{.ProjectKebab}} — first-time installer.
#
# Installs the {{.ProjectKebab}} binary from the hpcsc/{{.ProjectKebab}} GitHub
...
repo="hpcsc/{{.ProjectKebab}}"
binary_name="{{.ProjectKebab}}"
archive_name_base="{{.ProjectKebab}}"
...
api_releases() {
	...
	curl -fsSL --retry 3 \
		...
		-H 'user-agent: {{.ProjectKebab}}-install' \
		$auth_args \
...
choose_install_dir() {
	...
	printf 'Install {{.ProjectKebab}} to [%s]: ' "$default" >&2
	read -r custom || :
	if [ -n "$custom" ]; then
		default=$custom
	fi
...
# extract_binary unpacks the archive and finds the {{.ProjectKebab}} binary
...
print_done() {
	printf '\n{{.ProjectKebab}} %s installed to %s/%s\n' "$tag" "$install_dir" "$binary_name"
	case ":$PATH:" in
	*":$install_dir:"*) ;;
	*)
...
main() {
	...
	releases_json=$(api_releases) || die 'could not fetch the releases of hpcsc/{{.ProjectKebab}}'

	matches=$(versions_in_channel "$channel" "$releases_json")
	[ -n "$matches" ] || die "there is no $channel release of {{.ProjectKebab}} yet"

	printf '\nAvailable {{.ProjectKebab}} %s releases:\n' "$channel"
