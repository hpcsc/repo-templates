...
import (
	...
	"github.com/hpcsc/{{.ProjectKebab}}/internal/progress"
...
func newUpdateCommand() *cli.Command {
	return &cli.Command{
		Name:  "update",
		Usage: "replace {{.ProjectKebab}} with the latest release, or with the latest prerelease",
		Flags: []cli.Flag{
...
func update(ctx context.Context, cmd *cli.Command) error {
	...
	channel, command := release.Releases, "{{.ProjectKebab}} update"
	if cmd.Bool("prerelease") {
		channel, command = release.Prereleases, "{{.ProjectKebab}} update --prerelease"
	}
	...
	case !version.IsTagged(check.Current) && !cmd.Bool("force"):
		_, err = fmt.Fprintf(out, "{{.ProjectKebab}} %s is a build from a commit. The latest %s is %s.\n"+
			"Run %s --force to replace this build with it.\n", check.Current, channel, check.Latest.Tag, command)
		return err
	case check.UpToDate:
		_, err = fmt.Fprintf(out, "{{.ProjectKebab}} %s is the latest %s.\n", check.Current, channel)
		return err
	case cmd.Bool("check"):
		_, err = fmt.Fprintf(out, "{{.ProjectKebab}} %s is available. This is %s. Run %s to install it.\n", check.Latest.Tag, check.Current, command)
		return err
	}
	download := progress.Start(status, isTerminal(status), fmt.Sprintf("Downloading {{.ProjectKebab}} %s for %s", check.Latest.Tag, platform))
	err = updater.Install(ctx, check.Latest, download.Bytes)
	download.End()
	if err != nil {
		return err
	}
	_, err = fmt.Fprintf(out, "Updated {{.ProjectKebab}} from %s to %s at %s.\n", check.Current, check.Latest.Tag, executable)
	return err
}
