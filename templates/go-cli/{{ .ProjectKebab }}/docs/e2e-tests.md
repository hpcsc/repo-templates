...
The end-to-end tests run the {{.ProjectKebab}} binary in a terminal, send keys to it, and read the
...
sequenceDiagram
    ...
    Test->>Copy: {{.ProjectKebab}} update, with GITHUB_API_URL set to the fake API
    Copy->>API: GET /repos/hpcsc/{{.ProjectKebab}}/releases/latest
    ...
    Copy-->>Test: prints "the new {{.ProjectKebab}}"
```
...
```mermaid
flowchart LR
    subgraph builder["stage 1: golang:{{.Scaffold.GoVersion}}"]
        G["go build, with releaseTag set to v0.1.0"] --> BIN[/"{{.ProjectKebab}} binary"/]
    end
