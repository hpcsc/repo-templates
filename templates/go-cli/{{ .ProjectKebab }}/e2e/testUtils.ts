...
export function getExecutablePath(): string {
  ...
  if (!executablePath) {
    throw new Error('EXECUTABLE environment variable is required. Set it to the path of the {{.ProjectKebab}} binary.')
  }
...
export function buildTag(): string {
  ...
  if (!tag) {
    throw new Error('BUILD_TAG environment variable is required. Set it to the tag the {{.ProjectKebab}} binary was built with.')
  }
  return tag
}

export function scratchDir(): string {
  const dir = mkdtempSync(join(tmpdir(), '{{.ProjectKebab}}-e2e-'))
