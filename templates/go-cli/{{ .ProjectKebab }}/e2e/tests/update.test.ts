...
interface FakeRelease {
  tag: string
  prerelease: boolean
  // newBinary is the {{.ProjectKebab}} in the archive for this platform.
  newBinary: string
}
...
async function fakeReleases(releases: FakeRelease[]): Promise<string> {
  ...
  for (const release of releases) {
    ...
    writeFileSync(join(dir, '{{.ProjectKebab}}'), release.newBinary, { mode: 0o755 })
    const archive = `{{.ProjectKebab}}-${platform()}.tar.gz`
    execFileSync('tar', ['-czf', join(dir, archive), '-C', dir, '{{.ProjectKebab}}'])
  ...
  const server = createServer((request, response) => {
    ...
    if (path === '/repos/hpcsc/{{.ProjectKebab}}/releases/latest') {
      let latest = -1
    ...
    if (path === '/repos/hpcsc/{{.ProjectKebab}}/releases') {
      response.setHeader('content-type', 'application/json')
...
describe('{{.ProjectKebab}} update', () => {
  ...
  it('replaces the binary with the newer release', async () => {
    ...
    expect(update.stdout).toContain(`Updated {{.ProjectKebab}} from ${buildTag()} to v9.0.0`)
    const replaced = await runCli(scratchDir(), ['version'], {}, copy)
    expect(replaced.stdout).toContain('the new {{.ProjectKebab}}')
  })
  ...
  it('names each step on stderr while it updates, so that it does not look stuck', async () => {
    ...
    expect(update.stderr).toBe(
      `Finding the latest release of hpcsc/{{.ProjectKebab}}…\nDownloading {{.ProjectKebab}} v9.0.0 for ${platform()}…\n`,
    )
  })
  ...
  it('--prerelease replaces the binary with the latest prerelease', async () => {
    ...
    expect(update.status).toBe(0)
    expect(update.stdout).toContain(`Updated {{.ProjectKebab}} from ${buildTag()} to v9.0.1-7.gabc1234`)
    const replaced = await runCli(scratchDir(), ['version'], {}, copy)
  ...
  it('says so when the binary is the latest release', async () => {
    ...
    expect(result.status).toBe(0)
    expect(result.stdout).toContain(`{{.ProjectKebab}} ${buildTag()} is the latest release.`)
  })
})
